const API='https://api.cedarmc.org'; let products=[], account=null, cart=JSON.parse(localStorage.getItem('cedarmc-cart')||'[]'); const $=s=>document.querySelector(s);
const money=n=>'$'+Number(n||0).toFixed(2); async function api(path,opt={}){const r=await fetch(API+path,{credentials:'include',headers:{'Content-Type':'application/json',...(opt.headers||{})},...opt});let d={};try{d=await r.json()}catch{}if(!r.ok)throw new Error(d.error||'Request failed');return d}
function toast(t){const e=$('#toast');e.textContent=t;e.classList.add('show');setTimeout(()=>e.classList.remove('show'),1800)}
async function loadProducts(){try{products=await api('/api/store/products');renderProducts()}catch(e){$('#products').innerHTML='<p>Store products could not be loaded.</p>'}}
...
$('#accountBtn').onclick=openAccount;$('#heroAccount').onclick=openAccount;$('#closeAccount').onclick=closeAccount;$('#accountModal').onclick=e=>{if(e.target.id==='accountModal')closeAccount()};
$('#registerBtn').onclick=async()=>{try{const d=await api('/api/store/register',{method:'POST',body:JSON.stringify({email:$('#email').value,password:$('#password').value,minecraftUsername:$('#mcName').value,edition:$('#mcEdition').value})});account=d.account;await loadMe();toast('Account created')}catch(e){$('#authMessage').textContent=e.message}};
$('#loginBtn').onclick=async()=>{try{await api('/api/store/login',{method:'POST',body:JSON.stringify({email:$('#email').value,password:$('#password').value})});await loadMe();toast('Logged in')}catch(e){$('#authMessage').textContent=e.message}};
...
// loadMe() now refreshes the management link itself after /api/store/me finishes.
